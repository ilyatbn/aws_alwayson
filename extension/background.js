...
function fetchSts(roleArn, principalArn,SAMLReponse, props, port){
    ...
    fetch(STSUrl, {
        ...
        sset({'aws_sts_token':stsToken,'last_msg':'success'});
        if (port) port.postMessage('sts_ready');
    }).catch((error) => {
        sset({'last_msg':'sts_err'});
